...
double OptiLabCore::GainCell::bandLimitPd(double det,double thresh,double attack,double relBase,double relSlow,double pdAmt){
    if(gain<=0.0)gain=1.0; double depth=OptiLabCore::clamp((env-thresh)/std::max(thresh*2.5,tiny),0.0,1.0);
...
double OptiLabCore::EventCell::run(double det,double threshDb,double quietThreshold,double attack,double rate,double minimumRelease,double maximumRelease){
    ...
    if(env<0.00001)env=0.0;if(env>18.5){gain=0.12;return gain;}gain=OptiLabCore::clamp(OptiLabCore::dbToLin(-env),0.12,1.0);return gain;
}
...
OptiLabCore::Activity OptiLabCore::activity()const noexcept{return{std::min(currentAgcLowEffGain,currentAgcHighEffGain),currentDensityGain,currentFinalGain};}
void OptiLabCore::prepare(double newSampleRate){sampleRate=std::max(1.0,newSampleRate);lastMode=-1;applyModeAndDerivedParameters();reset();}
...
void OptiLabCore::applyModeAndDerivedParameters(){
    ...
    }else if(coreMode==1){inputTrimDb=4.5;phaseRotatePct=5;subsonicHpf=0;agcAmountPct=56;agcDriveDb=0;bassCouplingPct=50;releaseTime=5;gateThresholdDb=-30;gateReopenSpeedMs=50;gateReopenStrengthPct=55;densityDb=3.5;xt2AmountPct=84;bassEqDb=3.6;presenceDb=2.3;brillianceDb=3.2;clipDriveDb=-.8;clipperStyle=2;preLimiterPct=44;clipRestraintPct=38;clipCeilingDb=-.1;finalThresholdDb=-.8;finalThresholdMakeupPct=100;stereoMode=0;stereoWidthPct=100;topFilterMode=0;outputTrimDb=1.1;mbClipPct=16;dcCancelPct=46;overshootPct=100;recombControlPct=10;lookaheadMs=.54;crossoverModel=2;mbShapePct=0;mbLimiterDepthPct=50;mbCompLimiterPct=0;pdReleasePct=78;bassScReliefPct=76;lowCoherencePct=66;lowReleaseStabPct=74;transitionFillPct=12;lowBassFloorPct=42;adaptiveBassCouplingPct=65;adaptiveTopCouplingPct=50;bassClipPct=32;bassClipDensityPct=6;upperSnubberPct=20;snubberLookaheadMs=.73;postAgcSmoothDrivePct=12;postXt2SmoothDrivePct=12;preFinalDriveDb=-.4;smoothDriveRounderPct=50;
        const double widthStage=smoothstep01(autoUp/0.35);const double bassPressureStage=smoothstep01((autoUp-0.18)/(0.82-0.18));const double agcPressureStage=smoothstep01((autoUp-0.35)/(0.88-0.35));const double finalHandoff=smoothstep01((autoUp-0.50)/0.50);agcAmountPct=56+(50-56)*agcPressureStage;agcDriveDb=0.2*agcPressureStage;releaseTime=5+(6.1-5)*agcPressureStage;pdReleasePct=78;gateThresholdDb=-30-5*agcPressureStage;gateReopenStrengthPct=55;postAgcSmoothDrivePct=12+2*agcPressureStage;bassCouplingPct=50+(60-50)*bassPressureStage;bassEqDb=3.6;bassScReliefPct=76;lowBassFloorPct=42;lowCoherencePct=66+(82-66)*bassPressureStage;lowReleaseStabPct=74;adaptiveBassCouplingPct=65;bassClipPct=32+(40-32)*bassPressureStage;stereoWidthPct=100+50*widthStage;const double densityForkStage=smoothstep01((autoUp-0.50)/(0.78-0.50));const double broadcastShapeStage=smoothstep01((autoUp-0.50)/(0.80-0.50));const double legacyDensityDb=3.5+(3.8-3.5)*autoUp;densityDb=legacyDensityDb+(3.2-legacyDensityDb)*densityForkStage;mbClipPct=16+(24-16)*densityForkStage;mbCompLimiterPct=60*densityForkStage;mbShapePct=100*broadcastShapeStage;dcCancelPct=46+(18-46)*finalHandoff;upperSnubberPct=20+(68-20)*finalHandoff;postXt2SmoothDrivePct=12+(20-12)*finalHandoff;preFinalDriveDb=-.4+.4*finalHandoff;outputTrimDb=1.1*(1-finalHandoff);preLimiterPct=44+2*finalHandoff;clipRestraintPct=38-12*finalHandoff;finalStyleBlend=finalHandoff;finalThresholdDb=-.8+(-1.4+.8)*finalHandoff;const double legacyFinalLookahead=.54+(.70-.54)*autoUp;lookaheadMs=legacyFinalLookahead+(.54-legacyFinalLookahead)*finalHandoff;smoothDriveRounderPct=(50+(54-50)*autoUp)*(1-finalHandoff);
    ...
    masteringLookaheadSamples=processorMode==1?std::min(masterBufferLength-1,static_cast<int>(std::floor(sampleRate*.010+.5))):0;masterStartupPrimeWindow=static_cast<int>(std::floor(sampleRate*.350+.5));masterStartupActiveThresh=dbToLin(-70);pathDelay=static_cast<std::size_t>(masteringLookaheadSamples+effectiveSnubberLookahead+effectiveFinalLookahead+hybridTotalDelaySamples);
...
std::pair<float,float> OptiLabCore::processSample(float left,float right){
    double l=std::isfinite(left)?static_cast<double>(left):0.0,r=std::isfinite(right)?static_cast<double>(right):0.0;l=std::abs(l)<1e12?l:0;r=std::abs(r)<1e12?r:0;const double inputRawL=l,inputRawR=r;l*=inputGain;r*=inputGain;if(params.mode==Mode::StreamPolish){const double hpl=hp30L.hp(l,hpf30A),hpr=hp30R.hp(r,hpf30A);l+=(hpl-l)*streamHpfMix;r+=(hpr-r)*streamHpfMix;}else if(subsonicHpf){l=hp30L.hp(l,hpf30A);r=hp30R.hp(r,hpf30A);}if(phaseStages>0){for(int i=0;i<phaseStages;++i){l=apL[i].run(l,apC[i]);r=apR[i].run(r,apC[i]);}}
    ...
    if(producerDensityActive){
        ...
        if(activityTracking){std::array<double,6> blendedGain{};for(std::size_t i=0;i<6;++i)blendedGain[i]=compGain[i]*mbCompKeep+limiterGain[i]*mbCompLimiter;currentDensityGain=*std::min_element(blendedGain.begin(),blendedGain.end());}
    }else{
        pL=compProcessedL;pR=compProcessedR;
        if(activityTracking)currentDensityGain=*std::min_element(compGain.begin(),compGain.end());
    }
